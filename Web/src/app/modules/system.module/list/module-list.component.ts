...
export class ModuleListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.pageOption.filter = { ModuleCode: '', ModuleName: '' };
    this.pageOption.sort = { Collocation: 1 };
  ...
  create() {
    this.router.navigateByUrl('system-module/form/new');
  }

  showDetail(item) {
    this.router.navigateByUrl('system-module/form/' + item.ModuleId);
  }
  ...
  async delete(item) {
    if (await this.behaviors.confirm({
      ...
      this.loading = true;
      this.commonService.del(this.url, item.ModuleId).subscribe(
        res => {
          this.getMany();
        },
        err => {
  ...
  async deletes(itemSelected: Array<any>) {
    if (await this.behaviors.confirm({
      ...
      this.loading = true;
      this.commonService.dels(this.url, itemSelected.map(x => x.ModuleId)).subscribe(
        res => {
          this.getMany();
        },
        err => {
